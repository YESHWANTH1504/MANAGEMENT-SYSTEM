...
const InternProfiles = () => {
  ...
  useEffect(() => {
    if (selectedIntern) {
      ...
      fetchAttendanceRecords(selectedIntern.user_id);
      fetchWeeklySummary(selectedIntern.user_id);
                ...
                {loading ? (
                  ...
                  sortedInterns.map((intern) => (
                    <tr key={intern.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/20">
                      ...
                      <td className="px-6 py-4">
                        <p className="font-medium">{intern.start_date ? new Date(intern.start_date).toLocaleDateString() : 'N/A'}</p>
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {/* Profile Details (Academics & Skills) */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="glass-card p-6 hover-lift">
          <h4 className="text-xs font-black text-slate-800 dark:text-white uppercase tracking-wider mb-4 pb-2 border-b border-purple-500/10 flex items-center space-x-1.5">
            ...
            <span>Academic & Bio Details</span>
          </h4>
          <div className="space-y-3.5 text-xs">
            ...
            <div className="flex justify-between items-center py-1 border-b border-slate-100/50 dark:border-slate-800/50">
              ...
              <span className="font-extrabold text-slate-900 dark:text-white text-right">{(selectedIntern.degree || 'N/A')} ({(selectedIntern.department || 'N/A')})</span>
            </div>
            <div className="flex justify-between items-center py-1 border-b border-slate-100/50 dark:border-slate-800/50">
              ...
              <span className="font-extrabold text-slate-900 dark:text-white flex items-center justify-end space-x-1">
                ...
                <span>{selectedIntern.mobile_number || 'N/A'}</span>
              </span>
            </div>
            <div className="flex justify-between items-center py-1">
              <span className="text-slate-700 dark:text-slate-200 font-semibold">Duration</span>
              <span className="font-extrabold text-slate-900 dark:text-white">
                {selectedIntern.start_date ? new Date(selectedIntern.start_date).toLocaleDateString() : 'N/A'} - {selectedIntern.end_date ? new Date(selectedIntern.end_date).toLocaleDateString() : 'N/A'}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="glass-card p-6 hover-lift">
          <h4 className="text-xs font-black text-slate-800 dark:text-white uppercase tracking-wider mb-4 pb-2 border-b border-purple-500/10 flex items-center space-x-1.5">
            ...
            <span>Technical & Project Stack</span>
          </h4>
          <div className="space-y-3.5 text-xs">
            <div className="flex justify-between items-center py-1 border-b border-slate-100/50 dark:border-slate-800/50">
              <span className="text-slate-700 dark:text-slate-200 font-semibold">Active Project</span>
              <span className="font-extrabold text-slate-900 dark:text-white text-right truncate max-w-[180px]">{selectedIntern.project_name || 'N/A'}</span>
            </div>
            <div className="py-1 border-b border-slate-100/50 dark:border-slate-800/50">
              <p className="text-slate-700 dark:text-slate-200 font-semibold mb-1.5">Programming Languages</p>
              <div className="flex flex-wrap gap-1.5">
                {selectedIntern.programming_languages && selectedIntern.programming_languages.trim() ? selectedIntern.programming_languages.split(',').map((l, i) => (
                ...
                )) : <span className="text-slate-700 dark:text-slate-300">N/A</span>}
              </div>
            </div>
            <div className="py-1">
              <p className="text-slate-700 dark:text-slate-200 font-semibold mb-1.5">Frameworks & Tools</p>
              <div className="flex flex-wrap gap-1.5">
                {selectedIntern.frameworks && selectedIntern.frameworks.trim() ? selectedIntern.frameworks.split(',').map((f, i) => (
                ...
                {selectedIntern.tools_used && selectedIntern.tools_used.trim() ? selectedIntern.tools_used.split(',').map((t, i) => (
                  <span key={i} className="text-[9px] font-bold px-2 py-0.5 rounded bg-emerald-500/10 text-emerald-700 dark:text-emerald-300 capitalize">{t.trim()}</span>
                )) : null}
                {(!selectedIntern.frameworks || !selectedIntern.frameworks.trim()) && (!selectedIntern.tools_used || !selectedIntern.tools_used.trim()) && <span className="text-slate-700 dark:text-slate-300">N/A</span>}
              </div>
            </div>
          </div>
        </div>
      ...
      {activeTab === 'attendance' && (
        <AttendanceCalendar 
          ...
          startDate={selectedIntern.start_date}
        ...
        <div className="space-y-6 animate-fade-in">
          ...
          <div className="glass-card p-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-slate-150 dark:border-slate-800">
              ...
              <button
                onClick={() => generateWeeklySummary(selectedIntern.user_id)}
            ...
            {weeklySummaryLoading ? (
              <div className="flex flex-col items-center justify-center py-12 space-y-3">
                ...
                <p className="text-xs text-slate-550 dark:text-slate-400 animate-pulse">Consulting AI Model for contextual performance analysis...</p>
              </div>
            ) : weeklySummary ? (
          ...
          <div className="glass-card p-6">
            ...
            <div className="flex flex-col space-y-3">
              <p className="text-[10px] text-slate-500">Visualizes daily hours worked and attendance records (green density scale represents logged workload intensity)</p>
                    ...
                    return weeks.map((week, wIdx) => (
                      <div className="flex flex-col gap-1" key={wIdx}>
                        {week.map((day, dIdx) => {
                          ...
                          let colorClass = "bg-slate-100 dark:bg-slate-900 border border-slate-200/40 dark:border-white/5";
              ...
              {/* Heatmap Legend */}
              <div className="flex items-center justify-end space-x-1.5 text-[9px] text-slate-450 pt-2 font-bold uppercase tracking-wide">
                <span>Less</span>
                <div className="w-3 h-3 rounded-[2px] bg-slate-100 dark:bg-slate-900 border border-slate-200/40 dark:border-white/5" />
              ...
              <div className="h-60 w-full text-xs">
                {(() => {
                  ...
                  if (hoursTrend.length === 0) {
                    return <p className="text-slate-500 text-center py-20">No report data available to plot chart.</p>;
                  }
                  ...
                  return (
                    <ResponsiveContainer width="100%" height="100%">
                      <AreaChart data={hoursTrend} margin={{ top: 10, right: 10, left: -25, bottom: 0 }}>
                        ...
                        <Area type="monotone" dataKey="hours" name="Hours Worked" stroke="#8b5cf6" strokeWidth={2.5} fillOpacity={1} fill="url(#colorHours)" />
                      </AreaChart>
                    </ResponsiveContainer>
                  );
                })()}
            ...
            <div className="glass-card p-6">
              ...
              <div className="h-60 w-full text-xs">
                {(() => {
                  ...
                  if (totalCheckins === 0) {
                    return <p className="text-slate-550 text-center py-20">No arrival records available to plot chart.</p>;
                  }
